...
// Threshold values for different metal strengths and their corresponding changes in frequency
...
// Activates emergency stop of robot
void emergency_stop () {
	printf("E-stop :(\r\n");
	char buff[5];
	waitms(1000);
...
// return 1 if new char is ? -- execute emergency stop sequence
int check_estop() {
	char buff[4];
...
// Forward movement - blocking process
void forward (int cm) {
	timerCount_ms=0;
	...
	// take into account time required for acceleration up to a max velocity
	while (timerCount_ms < milliSeconds) {
		if (check_estop()) {
			e_stopped = 1;
			emergency_stop();
...
// CW (right turn) - blocking process
...
// CCW (left turn) - blocking process
...
// Move in a circle
void circle () {
	diag_left(390); // Full circle requires more than 360 degree turn, experimentally determined
}

// Drive in a square - blocking process
void square () { 
	for (int i = 0; i < 4; i++) {
		forward(20);
		stopCar_ms(10); 
...
// Switch statement that maps compressed letters to left wheel and right wheel pwms
...
/* Function: Decodes letters received
   Params:
	- letter -- encoded letter that is sent over
*/
void pathFindingDecoder (int letter) {
	// Switch statement range - hardcoded
	// If input character does not match with anything, then this will exit without calls
	switch (letter) {
...
int main(void)
{
	...
	while (1) {
		if (ReceivedBytes2()>0)
		{
			...
			// remote wants metal detector status
			if (buff[0]=='m') {

				count=GetPeriod(100);

				// Since we have the time of 200 periods, we need to divide by 200
				T= 1.0*count/(F_CPU*100.0); 
				f=1.0/T;
