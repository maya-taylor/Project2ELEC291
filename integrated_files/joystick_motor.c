/* FILE IN PROGRESS - COMBINATION OF MOTOR.C and WORKING-JOYSTICK.C */
/* modified scale to be from 65-100 */
...
void GetPosition2 (float volts[2], float pos[2]) {
	...
	if (pos[0] < mid_x) {
		pos[0] = map2(vx, 0.0, mid_x, -100.0, -65.0);
		printf("neg x\r\n");

	}
	else {
		pos[0] = map2(vx, mid_x, 3.3, 65.0, 100.0);
		printf("pos x\r\n");
	}

	if (pos[1] < mid_y) {
		pos[1] = map2(vy, 0.0, mid_y, -100.0, -65.0);
		printf("neg y\r\n");
	}
	else {
		pos[1] = map2(vy, mid_y, 3.3, 65.0, 100.0);
		printf("pos y\r\n");
	}
